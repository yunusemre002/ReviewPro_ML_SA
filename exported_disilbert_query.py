import pandas as pd
import numpy as np
reviews_df = pd.read_csv("Dataset/exported_disil_df.csv", encoding ="ISO-8859-1")  # Read data. Important!
df = reviews_df.copy()
print(df.shape)
df.info()

# df = df[df["Review Rating"] != 3]
df = df[df["Review Text"].apply(lambda x: len(x) < 2000)]
print("df: ", df.shape)
...
def printit(star):
    print(
            ...
            "\n 0.6 >      : ", df["Review Text"][(df['Review Rating'] == star) & (df.Score < 0.65)].count(),
